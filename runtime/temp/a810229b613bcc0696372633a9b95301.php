   ...
   <div class="right_side_content">
	   <div class="system_guanli_div">
        ...
        <div class="articleInfo_fill">
            <form id="form1" class="layui-form layui-form-pane" action='<?php echo url("get_role"); ?>'>
            ...
            <div class="course_manage_quanxian">
                <p><?php echo $list['title']; ?></p>
                <?php if(is_array($list['child']) || $list['child'] instanceof \think\Collection || $list['child'] instanceof \think\Paginator): if( count($list['child'])==0 ) : echo "" ;else: foreach($list['child'] as $key=>$child): ?>
                <dl class="course_manage_dl">
                    <dt>
                        <input type="checkbox" name="status[<?php echo $child['id']; ?>]" lay-filter="role" value="<?php echo $child['id']; ?>" <?php echo $child['checked']; ?> lay-skin="primary"><i> <?php echo $child['title']; ?></i>
                    </dt>
                    <?php if(isset($child['child'])): if(is_array($child['child']) || $child['child'] instanceof \think\Collection || $child['child'] instanceof \think\Paginator): if( count($child['child'])==0 ) : echo "" ;else: foreach($child['child'] as $key=>$three): ?>
                    <dd><input type="checkbox" name="status[<?php echo $three['id']; ?>]" lay-filter="role" value="<?php echo $three['id']; ?>" <?php echo $three['checked']; ?> lay-skin="primary"><i> <?php echo $three['title']; ?></i></dd>
