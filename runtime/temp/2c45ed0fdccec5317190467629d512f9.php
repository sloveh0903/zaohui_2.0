   ...
   <div class="right_side_content">
	   <div class="system_guanli_div">
	 		<ul class="system_guanli_ul">
                ...
                <li><a href="javascript:void(0)" class="active goto" data-src='<?php echo url("role/index"); ?>'>角色管理</a></li>
	    ...
	    <div class="course_right_main ">
	        <div class="operation_div">
	            <span class="span_add btn modal-catch" data-params='{"content":".add-subcat","act":"<?php echo url("add_role"); ?>", "title":"添加角色","type":"1","area":"500px,300px"}'>添加角色</span>
	            <span class="span_delete btn ajax-all" data-name="checkbox" data-params='{"url": "<?php echo url("delAll_role"); ?>","data":"","confirm":"true"}'>删除</span>
...
<div class="add-subcat">
    <form id="form1" class="layui-form layui-form-pane" action='<?php echo url("add_role"); ?>'>
...
<div class="edit-subcat" style="display: none">
    <form id="form2" class="layui-form layui-form-pane" action='<?php echo url("edit_role"); ?>'>
        <div class="layui-form-item">
...
<script id="list-tpl" type="text/html" data-params='{"url":"<?php echo url("role/index"); ?>","pageid":"#page"}'>
    <table id="example" class="layui-table" lay-skin="line">
        ...
        <tr>
            ...
            <td><input type="checkbox" name="switch" lay-skin="switch" lay-text="" {{#if (item.status){ }}checked="checked" {{# } }} lay-filter="ajax" data-params='{"url":"<?php echo url("roleStatus"); ?>","confirm":"true","data":"id={{ item.id}}"}'></td>
            <td>
                {{# if(item.id>1){ }}
                    <i class="i_edit goto" data-src="<?php echo url('get_role'); ?>?id={{ item.id}}">分配权限</i>／
                {{# } }}
                <i class="modal-catch i_edit" data-params='{"content": ".edit-subcat","act":"<?php echo url("edit_role"); ?>","title":"编辑角色","data":"id={{ item.id}}&title={{ item.title}}&status={{ item.status}}","type":"1","area":"500px,300px"}'>编辑</i>／<i class="i_delete ajax" data-params='{"url": "<?php echo url("del_role"); ?>","data":"id={{ item.id}}","confirm":"true"}'>删除</i>
            </td>
        </tr>
        {{# }); }}
        </tbody>
