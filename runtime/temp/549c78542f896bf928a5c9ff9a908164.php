     ...
     <div class="right_side_content">
	   	 <div class="system_guanli_div">
             <ul class="system_guanli_ul">
                 <li><a href="javascript:void(0)" class="goto active" data-src='<?php echo url("order/index"); ?>'>课程订单</a></li>
	    ...
	    <div class="course_right_main ">
	        <div class="operation_div">
	            <img class="img_add modal-catch" data-params='{"content":".add-subcat","act":"<?php echo url("add_order"); ?>", "title":"添加订单","type":"1","area":"500px,500px"}' src="/public/gzadmin/images/img_add@2x32.png" alt="添加图片">
	            <!--<img class="img_delete ajax-all" data-name="checkbox" data-params='{"url": "<?php echo url("delall_order"); ?>","data":"","confirm":"true"}' src="/public/gzadmin/images/img_delete@2x32.png" alt="删除图片">-->
	            <span class="span_batch_import btn modal-catch" data-params='{"content":".batch-add","act":"<?php echo url("excel/import"); ?>", "title":"批量导入","type":"1","area":"480px,360px"}'>批量导入</span>
	              <span class="span_batch_import btn modal-catch" data-params='{"content":".batch-export","act":"<?php echo url("excel/export"); ?>", "title":"批量导出","type":"1","area":"550px,300px"}'>批量导出</span>
	            <span class="download_template"><a href="/orderTpl.xlsx">下载模板</a></span>
	            <div class="select_search " >
	                <form class="layui-form" action='<?php echo url("order/index"); ?>'>
...
<!-- 	                                <input class="layui-input" name="start_date" placeholder="开始日"> -->
	                                <input type='hidden'  name="start_date" id="start_date" value=''>
              						<input class="layui-input" class="start_date" name="start_date" placeholder="报名开始日" id="LAY_start_date_s">
	                            </div>
	                            <div class="layui-input-inline" style="width: 120px;">
<!-- 	                                <input class="layui-input" name="end_date" placeholder="截止日"> -->
...
<div class="add-subcat">
    <form id="form1" class="layui-form layui-form-pane" action='<?php echo url("add_order"); ?>'>
        <div class="layui-form-item">
            <label class="layui-form-label">课程</label>
            <div class="layui-input-inline">
                <select name="course_id" jq-verify="required" jq-error="请选择课程" lay-filter="verify">
                    <option value=""></option>
                    <?php if(!empty($course)): if(is_array($course) || $course instanceof \think\Collection || $course instanceof \think\Paginator): if( count($course)==0 ) : echo "" ;else: foreach($course as $key=>$v): ?>
                    <option value="<?php echo $v['cid']; ?>"><?php echo $v['title']; ?></option>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <div class="layui-input-inline">
                    <input class="layui-input start-date" jq-verify="required" name="create_time" placeholder="购买时间">
                </div>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <div class="layui-input-inline">
                    <input class="layui-input end-date" jq-verify="required" name="expire_time"  placeholder="到期时间">
                </div>
            </div>
        </div>
        <div class="layui-form-item">
...
<div class="batch-add" style="display: none">
    <form id="form3" class="layui-form layui-form-pane" action='<?php echo url("excel/import"); ?>'>
        <div class="layui-form-item">
            <label class="layui-form-label">课程</label>
            <div class="layui-input-inline">
                <select name="cid" jq-verify="required" jq-error="请选择课程" lay-filter="verify">
                    <option value=""></option>
                    <?php if(!empty($course)): if(is_array($course) || $course instanceof \think\Collection || $course instanceof \think\Paginator): if( count($course)==0 ) : echo "" ;else: foreach($course as $key=>$v): ?>
                    <option value="<?php echo $v['cid']; ?>"><?php echo $v['title']; ?></option>
                    <?php endforeach; endif; else: echo "" ;endif; endif; ?>
...
<div class="edit-subcat" style="display: none">
    <form id="form2" class="layui-form layui-form-pane" action='<?php echo url("edit_order"); ?>'>
        <div class="layui-form-item">
            <label class="layui-form-label">课程</label>
            <div class="layui-input-inline">
                <select name="course_id" jq-verify="required" jq-error="请选择课程" lay-filter="verify">
...
<script id="list-tpl" type="text/html" data-params='{"url":"<?php echo url("order/index",["uid"=>$uid]); ?>","pageid":"#page"}'>
    <table id="example" class="layui-table" lay-skin="line">
        ...
        <tr>
            ...
            <td>
                <i class="userNick">{{ item.course_name}}</i>
            </td>
            <td>
            ...
            <td><i class="modal-catch i_edit"  data-remark="{{ item.remark}}" data-params='{"content": ".edit-subcat","act":"<?php echo url("edit_order"); ?>","title":"编辑订单","data":"id={{ item.id}}&course_id={{ item.course_id}}&create_time={{ item.createtime}}&expire_time={{ item.expire_time}}&mobile={{ item.mobile}}","type":"1","area":"500px,500px"}'>编辑</i><!-- <br><i class="i_delete ajax" data-params='{"url": "<?php echo url("del_order"); ?>","confirm":"true","data":"id={{ item.id}}"}'>删除</i> --></td>
        </tr>
        {{# }); }}
        </tbody>
