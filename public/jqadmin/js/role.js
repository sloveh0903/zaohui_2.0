...
layui.define(['jquery', 'form', 'jqform'], function(exports) {
    ...
    form.on('checkbox(role)', function(data) {
        //单击二级菜单
        if ($(data.elem).parent('dt').length > 0) {
            $(data.elem).parent('dt').parent('dl').find("dd").each(function(i, n) {
        ...
        if ($(data.elem).parent('dd').length > 0) {
            ...
            if (had_sub_check) {
                $(data.elem).parent('dd').parent('dl').find('dt').children('input').prop("checked", function() {
                    return data.elem.checked;
                });
            }
